...
{
    /// <summary>
    /// Controller that redirects fault information requests to XDA.
    /// </summary>
    [XDARedirect("api/Widgets/FaultInformation")]
    [RoutePrefix("api/EventWidgets/FaultInformation")]
    public class FaultInformationController : RedirectionController
    {
#if IS_GEMSTONE
        /// <summary>
        ...
        public FaultInformationController(IAPICredentialRetriever retriever) : base(retriever) { }
#endif

        [Route("{eventID:int}"), HttpGet]
        public async ServerResponse GetFaultInformation(int eventID, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);

        [Route("GetLinks/{category}"), HttpGet]
        public async ServerResponse GetLinks(string category, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);
    }
}
