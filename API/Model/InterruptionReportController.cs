...
{
    /// <summary>
    /// Controller that redirects interruption report requests to XDA.
    /// </summary>
    [XDARedirect("api/Widgets/InterruptionReport")]
    [RoutePrefix("api/EventWidgets/InterruptionReport")]
    public class InterruptionReportController : RedirectionController
    {
#if IS_GEMSTONE
        /// <summary>
        ...
        public InterruptionReportController(IAPICredentialRetriever retriever) : base(retriever) { }
#endif

        [Route("GetEvents/{hour:int}/{eventID:int}"), HttpGet]
        public async ServerResponse GetData(int hour, int eventID, CancellationToken token) =>
