...
{
    /// <summary>
    /// Controller that redirects file information requests to XDA.
    /// </summary>
    [XDARedirect("api/Widgets/FileInfo")]
    [RoutePrefix("api/EventWidgets/FileInfo")]
    public class FileInfoController : RedirectionController
    {
#if IS_GEMSTONE
        /// <summary>
        ...
        public FileInfoController(IAPICredentialRetriever retriever) : base(retriever) { }
#endif

        [Route("GetFileName/{eventID:int}"), HttpGet]
        public async ServerResponse GetFileInfo(int eventID, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);

        [Route("GetMappedChannels/{eventID:int}"), HttpGet]
        public async ServerResponse GetMappedChannels(int eventID, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);

        [Route("GetMeterConfiguration/{eventID:int}"), HttpGet]
        public async ServerResponse GetMeterConfiguration(int eventID, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);
    }
}
