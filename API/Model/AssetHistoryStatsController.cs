...
{
    /// <summary>
    /// Controller that redirects asset history statistics requests to XDA.
    /// </summary>
    [XDARedirect("api/Widgets/AssetHistoryStats")]
    [RoutePrefix("api/EventWidgets/AssetHistoryStats")]
    public class AssetHistoryStatsController : RedirectionController
    {
#if IS_GEMSTONE
        /// <summary>
        ...
        public AssetHistoryStatsController(IAPICredentialRetriever retriever) : base(retriever) { }
#endif

        [Route("{EventID:int}"), HttpGet]
        public async ServerResponse GetAssetHistoryStats(int EventID, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);

        [Route("{EventID:int}/{months:int}"), HttpGet]
        public async ServerResponse GetTimedAssetHistoryStats(int EventID, int months, CancellationToken token) =>
            await ForwardRequest(token).ConfigureAwait(false);
    }
}
