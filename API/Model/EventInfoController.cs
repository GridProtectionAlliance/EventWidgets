...
{
    /// <summary>
    /// Controller that redirects event information requests to XDA.
    /// </summary>
    [XDARedirect("api/Widgets/EventInfo")]
    [RoutePrefix("api/EventWidgets/EventInfo")]
#if IS_GEMSTONE
    // All endpoints proxy requests to XDA; Gemstone maps POST -> Create by verb.
    [ResourceAccess(ResourceAccessType.Read)]
#endif
    public class EventInfoController : RedirectionController
    {
#if IS_GEMSTONE
        /// <summary>
        ...
        public EventInfoController(IAPICredentialRetriever retriever) : base(retriever) { }
#endif

        [Route("{EventID:int}"), HttpGet]
        public async ServerResponse GetEventInfo(int EventID, CancellationToken token) =>
