...
const NoteWidget: EventWidget.IWidget<ISetting> = {
    ...
    Widget: (props: EventWidget.IWidgetProps<ISetting>) => {
        ...
        function getNoteType(): JQuery.jqXHR<OpenXDA.Types.NoteType[]> {
            const handle = $.ajax({
                type: "GET",
                url: `${props.HomePath}api/EventWidgets/Note/NoteType`,
        ...
        function getNoteApp(): JQuery.jqXHR<OpenXDA.Types.NoteApplication[]> {
            const handle = $.ajax({
                type: "GET",
                url: `${props.HomePath}api/EventWidgets/Note/NoteApp`,
                contentType: "application/json; charset=utf-8",
                dataType: 'json',
                cache: true,
                async: true
        ...
        function getNoteTag(): JQuery.jqXHR<OpenXDA.Types.NoteTag[]> {
            const handle = $.ajax({
                type: "GET",
                url: `${props.HomePath}api/EventWidgets/Note/NoteTag`,
                contentType: "application/json; charset=utf-8",
                dataType: 'json',
                cache: true,
                async: true
        ...
        function getIDs(): JQuery.jqXHR {
            const handle = $.ajax({
                type: "GET",
                url: `${props.HomePath}api/EventWidgets/Note/EventInformation/${props.EventID}`,
                cache: true,
                async: true,
            }).done((d) => {
                setIDs({
